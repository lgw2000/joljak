...
def check_cp_error(ticker,input):
    ...
    check_day_ago_sql = "SELECT day_ago FROM stock WHERE stock_ticker = %s"
    cur.execute(check_now_sql,(str(ticker)))
    result_check_now = cur.fetchall()
    cur.execute(check_day_ago_sql,(str(ticker)))
    result_check_day = cur.fetchall()
    result = ""
    result_check_now= float(result_check_now[0][0])
    result_check_day = float(result_check_day[0][0])
    if(result_check_now > result_check_day):
        result = "call"
    elif(result_check_now < result_check_day):
        result = "put"
    else:
        result = "0"
...
for i in check_stock:
    ...
    if(len(result_check_preded)==0):
        ...
        del_sqls = "DELETE FROM pred WHERE pred_len = 'day' AND pred_ticker = %s"
