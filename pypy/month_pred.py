...
for i in check_stock:
    ...
    dic_of_pred["input"] = []
    dic_of_pred["score"] = []
    check_preded = "SELECT * FROM pred WHERE pred_ticker = %s and pred_len = 'month'"
    cur.execute(check_preded,(str(i)))
    result_check_preded = cur.fetchall()
    if(len(result_check_preded)==0):
        #예측이 단 하나라도 없을때
        print("not preded   "+str(i))
        no_preds_sql = "UPDATE stock SET month_pn = 0 WHERE stock_ticker = %s"
        no_callputs_sql = "UPDATE stock SET month_cp = 0 WHERE stock_ticker = %s"
        ...
        for w in range(len(dic_of_pred["id"])):
            ...
            adds = adds + mul
        month_pn = str(adds/sum_of_score)
        month_cp = str(num_of_call)+":"+str(num_of_put)
        final_preds_sql = "UPDATE `stock` SET `month_pn` = %s WHERE `stock_ticker` = %s"
        final_callputs_sql = "UPDATE `stock` SET `month_cp` = %s WHERE `stock_ticker` = %s"
        cur.execute(final_preds_sql,(month_pn,str(i)))
        conn.commit()
        cur.execute(final_callputs_sql,(month_cp,str(i)))
        conn.commit()
