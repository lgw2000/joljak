...
def check_cp_error(ticker,input):
    ...
    check_month_ago_sql = "SELECT month_ago FROM stock WHERE stock_ticker = %s"
    cur.execute(check_now_sql,(str(ticker)))
    result_check_now = cur.fetchall()
    cur.execute(check_month_ago_sql,(str(ticker)))
    result_check_month = cur.fetchall()
    result = ""
    result_check_now= float(result_check_now[0][0])
    result_check_month = float(result_check_month[0][0])
    if(result_check_now > result_check_month):
        result = "call"
    elif(result_check_now < result_check_month):
        result = "put"
    else:
        result = "0"
...
for i in check_stock:
    ...
    if(len(result_check_preded)==0):
        ...
        del_sqls = "DELETE FROM pred WHERE pred_len = 'month' AND pred_ticker = %s"
