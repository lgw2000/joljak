...
for i in check_stock:
    ...
    dic_of_pred["input"] = []
    dic_of_pred["score"] = []
    check_preded = "SELECT * FROM pred WHERE pred_ticker = %s and pred_len = 'day'"
    cur.execute(check_preded,(str(i)))
    result_check_preded = cur.fetchall()
    if(len(result_check_preded)==0):
        #예측이 단 하나라도 없을때insert day
        print("not preded   "+str(i))
        no_preds_sql = "UPDATE stock SET day_pn = 0 WHERE stock_ticker = %s"
        no_callputs_sql = "UPDATE stock SET day_cp = 0 WHERE stock_ticker = %s"
        ...
        if sum_of_score <= 0:
            sum_of_score = float(len(dic_of_pred['id']))
        adds = 0
        for w in range(len(dic_of_pred["id"])):
            mul = dic_of_pred['score'][w] * dic_of_pred["input"][w]
            adds = adds + mul
        day_pn = str(adds/sum_of_score)
        day_cp = str(num_of_call)+":"+str(num_of_put)
        final_preds_sql = "UPDATE `stock` SET `day_pn` = %s WHERE `stock_ticker` = %s"
        final_callputs_sql = "UPDATE `stock` SET `day_cp` = %s WHERE `stock_ticker` = %s"
        cur.execute(final_preds_sql,(day_pn,str(i)))
        conn.commit()
        cur.execute(final_callputs_sql,(day_cp,str(i)))
        conn.commit()
